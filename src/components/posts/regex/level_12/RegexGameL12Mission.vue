...
<div>
    ...
    <p>
        We think we have cracked the Group's next plan. When we cross referenced the documents, images and the coordinates, one thing stands out. A non profit organization called ICANN. Its purpose is to manage DNS infrastructure including it's security. If you remember from our previous conversations about root servers, all of them fall under ICANN's purview. ICANN itself only manages 1 of them and has allocated rest of them to other organisations.
    </p>
    <p>

        As we have told you before, DNS network is like a telephone directory (or the contacts app) for computers. We give the computer a website like - "www.reinventingwheel.com" and the telephone directory returns the website's IP address. In the Phase 1 of the Project Blackout, The group planned to make this entire directory unavailable to the public. Now since that has failed, they are planning to counterfeit the directory and distribute it within the public, so whenever a user looks for a website, they'll be directed to something else.
    </p>
    <p>
        DNS infrastructure already has a security system built in to prevent counterfeiting. It is called DNSSEC. It works on a principal called public key cryptography. As the name suggests the system consists of 2 keys, one is private and is kept a secret while and the other is publicly given to anyone who asks for it. Sender of the information signs the data with his/her private key. The receiver can then use sender's public key to check the santity of the information. If someone tampers the information midway, then receiver would know and can take appropriate actions.
    </p>
    <p>
        At DNSSEC's heart sits a set of secret private keys. If The Group is able to get hold of these private keys, then they would be able to tamper with all the DNS traffic by intercepting communications and replacing them with malicious messages. Since they'd have signed the malicious information with the sender's private key, the receiver won't be able to detect that the message was tampered with.
    </p>
    <p>
    ...
    <p>
        To plan for extreme circumstances like destruction of the world, a copy of the keys is stored in a secure locker within the ICANN facilities. That locker in turn can only be accessed by a select few security experts. We have a list of those people below. Can you provide the name and country code of these people so that we can ensure their safety. We need your regex to be able to capture name of these experts individually.
    </p>
